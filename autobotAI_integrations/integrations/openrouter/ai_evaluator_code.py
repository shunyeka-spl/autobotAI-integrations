...
def executor(context):
    ...
    prompts.append({"role": "user", "content": "Resources: "})
    prompts.append({"role": "user", "content": f"Prompt: {prompt}"})

    current_prompt_len = 2300 + len(prompt)
    ...
    for resource in enumerate(resources):
        ...
        if current_prompt_len + resource_len < MAX_TOKEN * 3:
            prompts.append(
                {"role": "user", "content": json.dumps(resource, default=str)}
            )
