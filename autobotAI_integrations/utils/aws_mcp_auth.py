...
def is_aws_mcp_url(url: str) -> bool:
    """Return True when *url* targets the managed AWS MCP Server."""
    if not url:
        return False
...
def build_aws_mcp_creds(
    *,
    ...
    if default_aws_region:
        # Hint for operators/agents; AWS MCP tools also accept region per call.
...
def mcp_remote_server_aws_fields(creds: MCPCreds) -> Dict[str, Optional[str]]:
    """
    Extract optional AWS IAM fields from MCPCreds for MCPRemoteServer construction.
    """
    ...
    return {
        ...
        "aws_sigv4_region": creds.aws_sigv4_region,
