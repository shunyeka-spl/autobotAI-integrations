...
class TestClassAbuseipdb:
    def test_abuseipdb_token(
        self,
        get_keys,
        sample_integration_dict,
        test_result_format,
    ):
        tokens = {"api_key": get_keys["ABUSEIPDB_API_KEY"]}
        integration = sample_integration_dict("abuseipdb", tokens)
        ...
        tokens = {
            "api_key": get_keys["ABUSEIPDB_API_KEY"][:-3],
        }
        integration = sample_integration_dict("abuseipdb", tokens)
        service = integration_service_factory.get_service(None, integration)
        res = service.is_active()
    ...
    def test_actions_generation(self, get_keys):
        service = integration_service_factory.get_service_cls("abuseipdb")
